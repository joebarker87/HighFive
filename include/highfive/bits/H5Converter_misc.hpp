...
inline void vectors_to_single_buffer(const std::array<T, N>& vec_single_dim,
                                     const std::vector<size_t>& dims,
                                     size_t current_dim,
                                     std::vector<T>& buffer) {
...
template <typename T, std::size_t N>
inline void
vectors_to_single_buffer(const std::array<T, N>& arr_multi_dim,
                         const std::vector<size_t>& dims, size_t current_dim,
                         std::vector<typename type_of_array<T>::type>& buffer) {

  check_dimensions_vector(arr_multi_dim.size(), dims[current_dim],
          current_dim);
  for (typename std::array<T, N>::const_iterator it = arr_multi_dim.begin();
       it < arr_multi_dim.end(); ++it) {
    vectors_to_single_buffer(*it, dims, current_dim + 1, buffer);
  }
}
...
// apply conversion for std::array 1D
template <typename T, std::size_t N>
struct data_converter<
        std::array<T, N>,
        typename std::enable_if<(
                std::is_same<T, typename type_of_array<T>::type>::value)>::type> {
    inline data_converter(std::array<T, N>& arr, DataSpace& space, size_t dim = 0)
            : _space(&space), _dim(dim) {
      assert(_space->getDimensions().size() > dim);
      (void)arr;
    }
    ...
    inline typename type_of_array<T>::type*
    transform_read(std::array<T, N>& arr) {
      if (N != _space->getDimensions()[_dim]) {
        std::ostringstream ss;
        ss << "Mismatch between array size (" << N
           << ") and dataset size (" << _space->getDimensions()[_dim];
        ss << ") on dimension " << _dim;
        ss << " [std::array cannot be resized]";
        throw ObjectException(ss.str());
      }
      return &(arr[0]);
    }

    inline typename type_of_array<T>::type*
    transform_write(std::array<T, N>& arr) {
      return &(arr[0]);
    }

    inline void process_result(std::array<T, N>& arr) { (void)arr; }
...
// apply conversion for array nested arrays
template <typename T, std::size_t N>
struct data_converter<std::array<T, N>,
        typename std::enable_if<(is_container<T>::value)>::type> {
    inline data_converter(std::array<T, N>& vec, DataSpace& space, size_t dim = 0)
            : _dims(space.getDimensions()), _dim(dim), _vec_align() {
      (void)vec;
    }

    inline typename type_of_array<T>::type*
    transform_read(std::array<T, N>& vec) {
      (void)vec;
      _vec_align.resize(get_total_size());
      return &(_vec_align[0]);
    }
    ...
    inline typename type_of_array<T>::type*
    transform_write(std::array<T, N>& vec) {
      _vec_align.reserve(get_total_size());
      vectors_to_single_buffer<T>(vec, _dims, 0, _vec_align);
      return &(_vec_align[0]);
    }
